...
// ── Compact summary shown inline in onboarding ─────────────────────────────
          ...
          <div>
            ...
            <ul className="space-y-1">
              {summary.contraindications.slice(0, 3).map((c, i) => (
                <li key={i} className="text-[11px] text-slate/80 font-medium flex gap-1.5">
      ...
      {(summary.safePoses.length > 0 || summary.avoidPoses.length > 0) && (
        <div className="bg-white rounded-2xl border border-brand-border p-4 space-y-3">
          ...
          {summary.safePoses.length > 0 && (
            <div>
              <p className="small-caps text-[7px] text-gray-400 mb-1.5">Safe Poses</p>
              <div className="flex flex-wrap gap-1.5">
          ...
          {summary.avoidPoses.length > 0 && (
            <div>
              <p className="small-caps text-[7px] text-gray-400 mb-1.5">Avoid</p>
              <div className="flex flex-wrap gap-1.5">
...
export default function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  const analyzedSummary = docs.find(d => d.status === 'done')?.summary ?? null;
    ...
    <div className="min-h-full p-8 bg-brand-50 flex flex-col">
      ...
      <div className="mt-8 mb-8">
        <div className="flex gap-1 mb-6">
          {[1, 2, 3].map(s => (
            <div key={s} className="h-1 flex-1 rounded-full bg-brand-border overflow-hidden">
              <div
                className={cn(
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                onChange={e =>
                  setHealthData({ ...healthData, painLevel: parseInt(e.target.value) })
                }
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
            <p className="text-[12px] text-gray-500 leading-relaxed font-medium -mt-2">
              Upload prescriptions, MRI reports, or blood tests. Our AI reads them instantly
              and builds your personalised care profile.
            </p>
            ...
            {/* Upload zone — hide once docs are uploaded */}
            {docs.length === 0 && (
              <button
            ...
            {/* Add more — compact button shown after first upload */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="small-caps text-[7px] text-gray-400 mt-0.5">
                      {doc.status === 'processing'
                        ? 'Analysing with AI…'
                        : doc.status === 'done'
                        ? `Analysis complete · ${(doc.size / 1024).toFixed(0)} KB`
        ...
        {step === 3 && (
          ...
          >
            ...
            <p className="text-gray-500 text-xs max-w-xs mx-auto leading-relaxed">
              {analyzedSummary
                ? 'Your health profile and medical summary are ready. Your therapist will review them before your first session.'
            ...
            {analyzedSummary && (
              <div className="mt-6 flex items-center gap-2">
                ...
                <span className="small-caps text-[8px] text-forest">
                  Health Vault ready · {docs.filter(d => d.status === 'done').length} document
                  {docs.filter(d => d.status === 'done').length !== 1 ? 's' : ''} analysed
                </span>
              </div>
            )}
          </motion.div>
